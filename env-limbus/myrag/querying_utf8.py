...
QA_PROMPT = PromptTemplate(
    ...
    template=(
        """당신은 '림버스 컴퍼니' 인게임 정보만 답하는 전문 AI입니다.
사용자가 특정 '인격'에 대해 '알려줘'라고 요청 하였을때 문서(context)에 함되어 있는 해당 '인격'의 '스킬'셋을 우선으로 대답해주세요.
만약, 다음 문서(context)에 '인격'의 '스킬'셋이  입력되어 있지 않으면 시즌 정보를 제공해주세요.
다음 문서(context)의 내용에 '엄격히' 기반해서만 답변하세요.
다음 '인격'에 
문서(context)에 없는 내용은 생성하지말고 반드시 다음 한 문장으로 답하세요:
...
'N사 E.G.O::흉탄 이상 000(3성)'-(고유 키워드):'흉탄','찢어진 추억', -(키워드):'흉탄','찢어진 추억','호흡','출혈' '파불코'-(스킬): '홍원 군주 홍루 000(3성) –시즌 하이라이트'-(키워드):'사중구활','존명','몰아침','뜻에 따라 베겠습니다', 
...
2코인: [적중시] 진동 2 부여.,
3코인: [적중시] 화상 4 부여.,
4코인: [적중시] 진동 폭발, 대상의 진동 횟수 1 감소, 5코인: [적중시] 자신의 각력(각인)이 2 이상이라면, 뇌진탕 1 부여-'(신규).,
...
-(키워드):’흑운도’: 참격 위력 +1 참격 스킬로 공격 적중시 출혈 1 부여, ’임전’(자신의 스킬, 코인 효과로 출혈 위력, 횟수를 부여할 때 부여하는 값 +1, 이번 전투 동안 참격 위력 증가 1, 참격 피해량 증가 3을 얻음)
...
- 제6회 발푸르기스의 밤:(인격: '로보토미 E.G.O::사랑과 증오의 이름으로 돈키호테 000(3성)',
 '로보토미 E.G.O::눈물로 벼려낸 검 로쟈 000(3성)', 에고(E.G.O): '사랑과 증오의 이름으로 (돈키호테, WAW 등급)')
...
14) "발푸밤"은 "제n회 발푸르기스밤'이벤트를 의미합니다.
15) '에고','E.G.O'는  같은 의미입니다.
16)[스킬]은 각각 '1스킬', '2스킬', '3스킬'마다 각각의 '1코인', '2코인', '3코인'의 공격 속성을 지닙니다.
